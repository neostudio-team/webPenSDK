...
export default class PenClientParserV2 {
    ...
    switch (cmd) {
      ...
      case CMD.SETTING_INFO_RESPONSE:
        ...
        if (this.state.first) {
          ...
          this.PenController.onMessage(PenMessageType.PEN_SETTING_INFO, settingInfo);
      ...
      case CMD.SETTING_CHANGE_RESPONSE:
        ...
        this.PenController.onMessage(PenMessageType.PEN_SETUP_SUCCESS, settingChangeResult);
        break;
      ...
      case CMD.PASSWORD_RESPONSE:
        ...
        if (password.status === 1) {
          if (this.state.reCheckPassword) {
            this.PenController.onMessage(PenMessageType.PEN_SETTING_INFO, password);
          ...
          if (this.state.reCheckPassword) {
            ...
            this.PenController.onMessage(PenMessageType.PEN_PASSWORD_REQUEST, password);
          }
        }
        break;
      case CMD.PASSWORD_CHANGE_RESPONSE:
        ...
        if (packet.Result === 0x00) {
          ...
          this.PenController.onMessage(PenMessageType.PASSWORD_SETUP_FAILURE, passwordChange);
        }
        break;
      ...
      case CMD.OFFLINE_NOTE_LIST_RESPONSE:
        {
          ...
          this.PenController.onMessage(PenMessageType.OFFLINE_DATA_NOTE_LIST, noteList);
        }
        break;
      case CMD.OFFLINE_PAGE_LIST_RESPONSE:
        {
          let pageList = Res.PageList(packet)
          this.PenController.onMessage(PenMessageType.OFFLINE_DATA_PAGE_LIST, pageList);
        }
        break;
      ...
      case CMD.OFFLINE_DATA_DELETE_RESPONSE:
        ...
        this.PenController.onMessage(PenMessageType.OFFLINE_DATA_DELETE_RESPONSE, { Result: packet.Result === 0x00 });
        break;
  ...
  PaperInfoEvent(cmd, pk) {
    ...
    this.current.Owner = Converter.byteArrayToInt([rb[0], rb[1], rb[2], 0x00]);
  ...
  PenDotEvent(cmd, pk) {
    ...
    if (!this.penSettingInfo.HoverMode && !this.state.IsStartWithDown) {
      if (!this.state.IsStartWithPaperInfo) {
        ...
        let errorDot = Dot.MakeDot(this.current, x, y, force, Dot.DotTypes.PEN_ERROR, this.state.mPenTipColor, angel);
    ...
    if (this.penSettingInfo.HoverMode && !this.state.IsStartWithDown && this.state.IsStartWithPaperInfo) {
      dot = Dot.MakeDot(this.current, x, y, force, Dot.DotTypes.PEN_HOVER, this.state.mPenTipColor, angel);
  ...
  ResOfflineData(packet) {
    ...
    if (transPosition === 2) {
      ...
      this.PenController.onMessage(PenMessageType.OFFLINE_DATA_SEND_STATUS, 100);
    } else {
      ...
      this.PenController.onMessage(PenMessageType.OFFLINE_DATA_SEND_STATUS, percent);
    }
  }

  ParseSDK2OfflinePenData(u8, paper) {
    // NLog.log("OfflineStrokeParser", u8);
    let strokes = [];
    let packet = new Packet.PacketBuilder().data(u8).Build();
    ...
    for (let i = 0; i < strokeCount; i++) {
      ...
      for (let j = 0; j < dotCount; j++) {
        ...
        let dot = Dot.MakeDot(paper, x, y, force, dottype, penTipColor, angel);
        Dots.push(dot);
      }
      let stroke = { Dots };
      strokes.push(stroke);
    ...
    // NLog.log(strokes)
    this.PenController.onMessage(PenMessageType.OFFLINE_DATA_SEND_SUCCESS, strokes);
  }

  // NOTE: Request(Offline Receive Response)
  ReqOfflineData2(index, isSuccess = true, end = false) {
