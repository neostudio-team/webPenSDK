const ByteUtil = require('./byte_util')
const Packet = require('./packet')
const Converter = require('./converter')
const Dot = require('./dot')

class PenClientParserV2 {
	...
	ParsePacket(packet) {
		...
		switch (cmd) {
			case this.Cmd.VERSION_RESPONSE:
				...
				if (isMG && this.DeviceName === this.F121 && this.SubName === "Mbest_smartpenS")
			...
			case this.Cmd.SETTING_INFO_RESPONSE:
				{
					...
					if (this.MaxForce === -1) {
						...
						if (lockyn) {
							this.PenController.onPenPasswordRequest({ 'RetryCount': pwdRetryCount, 'ResetCount': pwdMaxRetryCount });
						}
						else {
			...
			case this.Cmd.SETTING_CHANGE_RESPONSE:
				{
					...
					switch (stype) {
						case this.SettingType.AutoPowerOffTime:
							this.PenController.onPenAutoShutdownTimeSetupResponse({ 'Result': result });
							break;

						case this.SettingType.AutoPowerOn:
							this.PenController.onPenAutoPowerOnSetupResponse({ 'Result': result });
							break;
						...
						case this.SettingType.OfflineData:
							this.PenController.onPenOfflineDataSetupResponse({ 'Result': result });
							break;

						case this.SettingType.PenCapOff:
							this.PenController.onPenCapPowerOnOffSetupResponse({ 'Result': result });
							break;

						case this.SettingType.Sensitivity:
							this.PenController.onPenSensitivitySetupResponse({ 'Result': result });
							break;

						case this.SettingType.Timestamp:
							this.PenController.onPenTimestampSetupResponse({ 'Result': result });
							break;
					}
				}
				break;
			...
			case this.Cmd.PASSWORD_RESPONSE:
				{
					...
					if (status === 1) {
						...
						if (this.reCheckPassword) {
							...
							this.PenController.onPenPasswordRequest({ 'RetryCount': cntRetry, 'ResetCount': cntMax });
						}
					}
				}
				break;
			...
			case this.Cmd.OFFLINE_PAGE_LIST_RESPONSE:
				{
					...
					let info = { 'Section': section, 'Owner': owner, 'Note': note, 'Pages': pages };
			...
			case this.Cmd.OFFLINE_DATA_DELETE_RESPONSE:
				{
					this.PenController.onRemovedOfflineData({ 'Result': packet.Result === 0x00 });
				}
				break;
	...
	CheckEventCount(ecount) {
		...
		if (ecount - this.EventCount !== 1 && (ecount !== 0 || this.EventCount !== 255)) {
			let errorDot = null;
			...
			if (ecount - this.EventCount > 1) {
				let extraData = "missed event count " + (this.EventCount + 1) + "-" + (ecount - 1);
				this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.InvalidEventCount, 'Dot': errorDot, 'Timestamp': this.SessionTs, 'ExtraData': extraData });
			}
			...
			case this.Cmd.ONLINE_NEW_PEN_DOWN_EVENT:
				{
					...
					this.mPenTipType = pk.GetByte() == 0x00 ? this.PenTipType.Normal : this.PenTipType.Eraser;
			...
			case this.Cmd.ONLINE_NEW_PEN_UP_EVENT:
				{
					...
					if (this.IsStartWithDown && this.IsBeforeMiddle && this.mPrevDot !== null) {
						...
						this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPenDownPenMove, 'Timestamp': -1 });
					}
					else if (!this.IsBeforeMiddle) {
						// 무브없이 다운-업만 들어올 경우 UP dot을 보내지 않음
						this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPenMove, 'Timestamp': this.SessionTs });
					}
			...
			case this.Cmd.ONLINE_NEW_PEN_ERROR_EVENT:
				{
					...
					*/
					...
					this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.ImageProcessingError, 'Dot': errorDot, 'Timestamp': this.SessionTs, 'ImageProcessErrorInfo': newInfo });
				}
				break;
		}
	}
	...
	PaperInfoEvent(cmd, pk) {
		if (cmd === this.Cmd.ONLINE_NEW_PAPER_INFO_EVENT || cmd === this.Cmd.ONLINE_ENCRYPTION_PAPER_INFO_EVENT) {
			let ecount = pk.GetByteToInt();
	...
	PenDotEvent(cmd, pk) {
		if (cmd === this.Cmd.ONLINE_NEW_PEN_DOT_EVENT || cmd === this.Cmd.ONLINE_ENCRYPTION_PEN_DOT_EVENT) {
			let ecount = pk.GetByteToInt();
		...
		if (!this.HoverMode && !this.IsStartWithDown) {
			if (!this.IsStartWithPaperInfo) {
				...
				this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPenDown, 'Timestamp': -1 });
			}
			else {
				this.mTime = Date.now()
				...
				let errorDot = MakeDot(this.PenMaxForce, this.mCurOwner, this.mCurSection, this.mCurNote, this.mCurPage, this.mTime, x, y, fx, fy, force, Dot.DotTypes.PEN_ERROR, this.mPenTipColor);

				//펜 다운 없이 페이퍼 정보 있고 무브가 오는 현상(다운 - 무브 - 업 - 다운X - 무브)
				this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPenDown, 'Dot': errorDot, 'Timestamp': this.SessionTs });
		...
		if (this.HoverMode && !this.IsStartWithDown && this.IsStartWithPaperInfo) {
			dot = this.MakeDot(this.PenMaxForce, this.mCurOwner, this.mCurSection, this.mCurNote, this.mCurPage, this.mTime, x, y, fx, fy, force, Dot.DotTypes.PEN_HOVER, this.mPenTipColor);
		}
		else if (this.IsStartWithDown) {
			if (this.IsStartWithPaperInfo) {
				dot = this.MakeDot(this.PenMaxForce, this.mCurOwner, this.mCurSection, this.mCurNote, this.mCurPage, this.mTime, x, y, fx, fy, force, this.mDotCount == 0 ? Dot.DotTypes.PEN_DOWN : Dot.DotTypes.PEN_MOVE, this.mPenTipColor);
			}
			else {
				//펜 다운 이후 페이지 체인지 없이 도트가 들어왔을 경우
				this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPageChange, 'Timestamp': this.SessionTs });
			}
		}
	...
	ParseDot(mPack, type) {
		...
		this.ProcessDot(this.MakeDot(this.PenMaxForce, this.mCurOwner, this.mCurSection, this.mCurNote, this.mCurPage, this.mTime, x, y, fx, fy, force, type, this.mPenTipColor), null);
	...
	MakeUpDot(isError = true) {
		if (isError) {
			...
			errorDot.DotType = Dot.DotTypes.PEN_ERROR;
			this.PenController.onErrorDetected({ 'ErrorType': this.ErrorType.MissingPenUp, 'Dot': errorDot, 'Timestamp': this.SessionTs });
		}
	...
	Escape(input) {
		if (input === this.Const.PK_STX || input === this.Const.PK_ETX || input === this.Const.PK_DLE) {
	...
	ReqVersion() {
		...
		let StrProtocolVersion = Converter.toUTF8Array(this.SupportedProtocolVersion);
	...
	RequestChangeSetting(stype, value) {
		...
		switch (stype) {
			...
			case this.SettingType.LedColor:
				...
				bf.PutShort(5).Put(stype).PutArray(nBytes.buffer, 4);
			...
			case this.SettingType.DownSampling:
				bf.PutShort(2).Put(stype).Put((value ? 1 : 0));
				break;
			case this.SettingType.Sensitivity:
	...
	ReqSetupDataTransmissionType(type) {
		return this.RequestChangeSetting(this.SettingType.DataTransmissionType, type)
	}
	...
	SendAddUsingNote(sectionId = -1, ownerId = -1, noteIds = null) {
		let bf = new ByteUtil(this.Escape.bind(this));
		bf.Put(this.Const.PK_STX, false)
			.Put(this.Cmd.ONLINE_DATA_REQUEST);
		...
		if (sectionId >= 0 && ownerId > 0 && noteIds == null) {
			...
			bf.PutShort(length)
				.PutShort(noteIds.Length);

			foreach(item in noteIds)
			{
				bf.Put(this.GetSectionOwnerByte(sectionId, ownerId))
					.PutInt(item);
			}
		}
		else {
	...
	SendAddUsingNotes(sectionId, ownerId) {
		...
		bf.PutShort((2 + sectionId.Length * 8))
			.PutShort(sectionId.Length);
	...
	ReqAddUsingNote() {
		return this.SendAddUsingNote();
	}

	ReqAddUsingNote(section, owner, notes = null) {
		return this.SendAddUsingNote(section, owner, notes);
	}

	ReqAddUsingNotes(section, owner) {
		return this.SendAddUsingNote(section, owner);
	}

	//
	// Offline Data
	// 
	ReqOfflineDataList(section = -1, owner = -1) {
		let bf = new ByteUtil(this.Escape.bind(this));

		let pInfo = section > 0 && owner > 0 ? GetSectionOwnerByte(section, owner) : new Uint8Array([0xFF, 0xFF, 0xFF, 0xFF]).buffer
	...
	ReqRemoveOfflineData(section, owner, notes) {
		...
		bf.Put(this.Const.PK_STX, false)
			.Put(this.Cmd.OFFLINE_DATA_DELETE_REQUEST);

		let length = (short)(5 + (notes.Length * 4));

		bf.PutShort(length)
			.Put(this.GetSectionOwnerByte(section, owner))
			.Put(notes.Length);

		foreach(noteId in notes)
		{
			bf.PutInt(noteId);
		}
	...
	//public Dot( 
	MakeDot(penMaxForce, owner, section, note, page, timestamp, x, y, fx, fy, force, type, color) {
		let builder = null;
		if (penMaxForce == 0) builder = new Dot.DotBuilder();
		else builder = new Dot.DotBuilder(penMaxForce);
	...
	isF121MG(macAddress) {
		...
		let mgStart = Converter.byteArrayToLong(MG_F121_MAC_START.replace("/:/g", ""), 16);
		let mgEnd = Converter.byteArrayToLong(MG_F121_MAC_END.replace("/:/g", ""), 16);
...
PenClientParserV2.prototype.PEN_PROFILE_SUPPORT_PROTOCOL_VERSION = Object.freeze(2.10)
...
module.exports = PenClientParserV2
